    ...
    pub fn from_xml_database(alloc: Allocator, xml_database: *const XmlDatabase) !TypeDatabase {
        ...
        for (xml_database.types.handles) |*handle| {
            if (handle.alias) |alias| {
                ...
                const h: Handle = .{ .name = handle.name };
                ...
                .enum_name => |enum_name| {
                    if (xml_database.enum_by_name(enum_name)) |e| {
                        ...
                        for (xml_database.features.items) |ext| {
                            const extensions = try ext.enum_extensions(alloc, e.name);
                            for (extensions.items) |ex| {
                                switch (ex.value) {
                                    .bitpos => |bit| {
                                        try bits.append(
                                            alloc,
                        ...
                        const b: Bitfield = .{
                            .name = bitmask.name,
                            .backing_integer_width = e.bitwidth,
                            .bits = bits.items,
        ...
        for (xml_database.enums.items) |*@"enum"| {
            if (@"enum".type == .@"enum") {
                ...
                for (xml_database.features.items) |ext| {
                    const extensions = try ext.enum_extensions(alloc, @"enum".name);
                    for (extensions.items) |ex| {
                        switch (ex.value) {
                            .value => |value| {
                                try values.append(
...
pub const XmlDatabase = struct {
    ...
    pub const Extension = struct {
        ...
        pub const EnumExtension = union(enum) {
            value: i32,
            offset: struct {
                offset: u32,
                extnumber: u32,
